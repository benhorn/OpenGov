...
public class KeywordPublicationServiceImpl extends BaseLog implements KeywordPublicationService{
	...
	@Override
	public boolean updateKeywordPublicaion(KeywordPublication keyPub)
			throws Exception {
		Session session = HibernateUtil.getSession();
		Transaction tx = null;
		boolean result = false;
		try{
			tx = session.beginTransaction();
			session.update(keyPub);
			tx.commit();
			result=true;
		}catch(Exception ex){
			if(tx!=null)tx.rollback();
			logger.info("In class KeywordPublicationServiceImpl:updateKeywordPublicaion()\n");
			ex.printStackTrace();
		}finally{
			HibernateUtil.closeSession();
		}	
	...
	/*public static void main(String[] args) throws Exception
	{
		...
		Publication publication = ps.loadPublication(120);
		
		
		kps.deleteKeyPubViaPublication(publication);
		
		//System.out.println(status);
		//kps.deleteKeyPubViaPublication("115");
