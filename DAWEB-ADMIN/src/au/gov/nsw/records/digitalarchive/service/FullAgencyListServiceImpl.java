...
public class FullAgencyListServiceImpl extends BaseLog implements FullAgencyListService{
	...
	{
		FullAgencyListService fs = new FullAgencyListServiceImpl();
		List<AgencyPublication> fList = fs.loadAgencyWithPublication();
		AgencyPublication thatAP = null;
			
		//System.out.println(fList.size());
		int i = 0;
		Iterator<AgencyPublication> it = fList.iterator();
		while(it.hasNext())
		{
			thatAP = (AgencyPublication)it.next();
			System.out.println("" + thatAP.getFullAgencyList().getAgencyName());
			i++;
		}
