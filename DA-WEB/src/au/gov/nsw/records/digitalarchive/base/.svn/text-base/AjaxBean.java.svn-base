...
public class AjaxBean extends BaseLog {
	...
	public String[][] getAgencies(){
		...
		try{
			List<BosList> list = service.browseBosList();
			BosList bosList = null;
			int i = 0;
			if (list!=null)
			{
				options = new String[list.size()][2];
				Iterator<BosList> it = list.iterator();
				while(it.hasNext())
				{
	...
	public Integer countFiles(String pubID) 
	{
		FileService fs = new FileServiceImpl();
		Integer count = 0;
		try {
			count = fs.countFiles(Integer.parseInt(pubID));
		} catch (Exception e) {
			logger.info("In class AjaxBean:countFiles()\n");
			e.printStackTrace();
		}
		return count;
	}
